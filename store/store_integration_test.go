...
func TestGetGCRAuth_GCRCredsAbsent(t *testing.T) {
	creds := &dockerCredentials{
		OtherCreds: map[string]credentials.Credentials{
			"whatever.io": {
				Username: "foobarre",
				Secret:   "not really",
			},
...
func TestGetOtherCreds_HappyCase(t *testing.T) {
	const (
		testReg          = "whatever.io"
		expectedUsername = "foobarre"
		expectedSecret   = "not really"
	)
	creds := &dockerCredentials{
		OtherCreds: map[string]credentials.Credentials{
			testReg: {
	...
	actualCreds, err := tested.GetOtherCreds(testReg)
	if err != nil {
		t.Fatalf("GetOtherCreds returned an error: %v", err)
	}
	actualUser := actualCreds.Username
	if actualUser != expectedUsername {
		t.Errorf("Expected username \"%s\" but got \"%s\"", expectedUsername, actualUser)
	}
	actualSecret := actualCreds.Secret
	if actualSecret != expectedSecret {
		t.Errorf("Expected secret \"%s\" but got \"%s\"", expectedSecret, actualSecret)
	}
}

func TestGetOtherCreds_NotPresent(t *testing.T) {
	const testReg = "whatever.io"
	creds := &dockerCredentials{
		OtherCreds: map[string]credentials.Credentials{
			"somewhere.else": {
				Username: "more like LOSERname",
	...
	actualCreds, err := tested.GetOtherCreds(testReg)
	if err == nil {
		t.Fatalf("Expected an error, got: %v", actualCreds)
	}
}

func TestGetOtherCreds_NoFile(t *testing.T) {
	err := cleanUp()
	if err != nil {
		t.Fatal("Could not guarantee that no credential file existed.")
	}
	...
	actualCreds, err := tested.GetOtherCreds("anything.com")
	if err == nil {
		t.Fatalf("Expected an error, got: %+v", actualCreds)
	}
}

func TestSetOtherCreds_Overwrite(t *testing.T) {
	const (
		testReg          = "whatever.io"
		expectedUsername = "foobarre"
		expectedSecret   = "not really"
	...
	err := tested.SetOtherCreds(&credentials.Credentials{
		ServerURL: testReg,
		Username:  expectedUsername,
		Secret:    expectedSecret,
	})
...
func TestSetOtherCreds_NewAddition(t *testing.T) {
	const (
		testReg          = "whatever.io"
		expectedUsername = "foobarre"
		expectedSecret   = "not really"
...
func TestSetOtherCreds_NoFile(t *testing.T) {
	err := cleanUp()
	if err != nil {
		t.Fatal("Could not guarantee that no credential file existed.")
	}
...
func TestDeleteOtherCreds(t *testing.T) {
	testReg := "whatever.io"
	creds := &dockerCredentials{
		OtherCreds: map[string]credentials.Credentials{
			testReg: {
	...
	err := tested.DeleteOtherCreds(testReg)
	if err != nil {
		t.Fatalf("DeleteOtherCreds returned an error: %v", err)
	}
...
func TestDeleteOtherCreds_NoFile(t *testing.T) {
	err := cleanUp()
	if err != nil {
		t.Fatal("Could not guarantee that no credential file existed.")
	}
	...
	err = tested.DeleteOtherCreds("whatever.com")
	if err != nil {
		t.Fatalf("DeleteOtherCreds returned an error: %v", err)
	}
}

func TestDeleteOtherCreds_NotPresent(t *testing.T) {
	testReg := "whatever.io"
	creds := &dockerCredentials{
		OtherCreds: map[string]credentials.Credentials{
			"other.reg": {
...
func TestOtherCredsLifespan(t *testing.T) {
	err := cleanUp()
	if err != nil {
		t.Fatal("Could not guarantee that no credential file existed.")
	}
